const CartRepository = {
    ...
    try {
      db.withTransactionSync(() => {
        const headerResult = db.runSync(
          "INSERT INTO cart (id, date, customer_id, total) VALUES (?,?,?,?)",
        ...
        if (headerResult.changes > 0) {
          ...
          cart.items.forEach((item) => {
            const detailResult = db.runSync(
              "INSERT INTO cart_detail (id, cart_id, item_id, quantity) VALUES (?,?,?,?)",
  ...
  getAllCart: (db) => {
    try {
      return db.getAllSync(
        `SELECT c.*, cs.name, cs.address FROM cart c LEFT JOIN customers cs ON c.customer_id = cs.id ORDER BY c.id DESC`,
      );
    } catch (error) {
  ...
  getAllCartDetail: (db) => {
    try {
      return db.getAllSync(
        "SELECT cd.*, i.* FROM cart_detail cd, items i WHERE cd.item_id = i.id ORDER BY cd.cart_id DESC",
      );
    } catch (error) {
