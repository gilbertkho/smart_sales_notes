...
const CartScreen = () => {
  ...
  const sendToPOS = async (cart) => {
    ...
    try {
      let responseBarang = await axios.post(
        `http://${baseUrl}/pos-agape/smart/sync`,
        cart,
      );
          ...
          <View style={{ ...stylesModal.modalView, gap: 10 }}>
            ...
            <Text style={{ textAlign: "center" }}>
              ...
              <Text style={{ fontWeight: "bold" }}>
                {" "}
                {thousandSeparator(selectedCart.total)}
              </Text>
              ?
            </Text>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={cartStyles.cartItems}>
            ...
            >
              <Text style={{ ...styles.cartItemTitle, fontWeight: "bold" }}>
                {item.date}
              </Text>
              <TouchableOpacity
                style={{
            ...
            <Text style={{ ...styles.cartItemText, fontWeight: "bold" }}>
              {item.name}
            </Text>
            <Text style={{ ...styles.cartItemText }}>{item.address}</Text>
            <Text style={{ ...styles.cartItemText }}>
              Total: {thousandSeparator(item.total)}
            </Text>
            <View
              style={{
                display: "flex",
            ...
            >
              <TouchableOpacity
                ...
                disabled={
                  item.id == null || item.id == "" || !Object.hasOwn(item, "id")
                    ? true
                    : false
                }
                onPress={() => {
              ...
              <TouchableOpacity
                style={[
                  item.is_sent === 0 ? styles.bgSuccess : styles.bgPrimary,
                  {
                    flexGrow: 1,
                    height: 40,
                    display: "flex",
